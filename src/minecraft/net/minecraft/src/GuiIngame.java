...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
        ...
        /*drawRect(130, 1, 1, 265, 0xaf424242);
        fontrenderer.drawStringWithShadow("Le nom de votre client", 2, 2, 0xffff);
        fontrenderer.drawStringWithShadow("Up = \2474Menu\247f", 2, 12, 0xffff);*/
    ...
    /**
     *  Render de la barre d'alignement.
     */
    
    private void renderAlignement()
    {
    	if (RenderDragon.entityDragon != null)
        {
            return;
        ...
        char c = '\266';
        int j = i / 2 - c / 2;
        int k = (int)(((float)EntityPlayer.Alignement / (float)180) * (float)(c + 1));
        byte byte0 = 12;
        drawTexturedModalRect(j, byte0, 0, 84, c, 5);
        drawTexturedModalRect(j, byte0, 0, 84, c, 5);

        if (k > 0)
        {
            drawTexturedModalRect(j, byte0, 0, 89, k, 5);
        }

        String s = "Alignement: " + EntityPlayer.Alignement;
        fontrenderer.drawStringWithShadow(s, i / 2 - fontrenderer.getStringWidth(s) / 2, byte0 - 10, EntityPlayer.getCouleur());
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, mc.renderEngine.getTexture("/gui/icons.png"));
    }
